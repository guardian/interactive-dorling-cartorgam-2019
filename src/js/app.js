...
window.updateData = function updateData(button)
{
    ...
    datum.map((d,i) => {
      nodes[i].country= d.country
       nodes[i].continent= d.continent
       nodes[i].x= projection([d.latitude, d.longitude])[0], nodes[i].y= projection([d.latitude, d.longitude])[1]
       nodes[i].x0= projection([d.latitude, d.longitude])[0], nodes[i].y0= projection([d.latitude, d.longitude])[1]
       nodes[i].value= +d["total " + button.name]
      nodes[i].r = radius(+d["total " + button.name])
    })
    ...
    simulation = d3.forceSimulation()
      ...
      .force("x", d3.forceX(function(d) { return d.x0; }))
      .force("y", d3.forceY(function(d) { return d.y0; }))
      .force("collide", d3.forceCollide().strength(1).radius(d => d.r + 1))
      .nodes(nodes)
      .alphaTarget(0)
      .on("tick", tick)
      .on('end', function() {
